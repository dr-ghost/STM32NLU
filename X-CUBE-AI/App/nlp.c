...
#define AI_NET_OBJ_INSTANCE g_nlp
 
#undef AI_NLP_MODEL_SIGNATURE
#define AI_NLP_MODEL_SIGNATURE     "0x0f2a872c698530e3514e7b43e9a50151"
...
#define AI_TOOLS_DATE_TIME   "Sun May  5 04:59:41 2024"

#undef AI_TOOLS_COMPILE_TIME
#define AI_TOOLS_COMPILE_TIME    __DATE__ " " __TIME__

#undef AI_NLP_N_BATCHES
#define AI_NLP_N_BATCHES         (1)

static ai_ptr g_nlp_activations_map[1] = AI_C_ARRAY_INIT;
static ai_ptr g_nlp_weights_map[1] = AI_C_ARRAY_INIT;
...
AI_ARRAY_OBJ_DECLARE(
  serving_default_dense_32_input0_output_array, AI_ARRAY_FORMAT_FLOAT|AI_FMT_FLAG_IS_IO,
...
AI_TENSOR_OBJ_DECLARE(
  ...
  1, 0x0,
  AI_SHAPE_INIT(4, 6, 3, 1, 1), AI_STRIDE_INIT(4, 4, 24, 72, 72),
...
AI_TENSOR_OBJ_DECLARE(
  ...
  4, 0x0,
  AI_SHAPE_INIT(4, 10, 20, 1, 1), AI_STRIDE_INIT(4, 4, 40, 800, 800),
...
AI_TENSOR_OBJ_DECLARE(
  ...
  8, 0x0,
  AI_SHAPE_INIT(4, 10, 6, 1, 1), AI_STRIDE_INIT(4, 4, 40, 240, 240),
  1, &gemm_2_weights_array, NULL)

/* Tensor #9 */
AI_TENSOR_OBJ_DECLARE(
  serving_default_dense_32_input0_output, AI_STATIC,
  9, 0x0,
  AI_SHAPE_INIT(4, 1, 10, 1, 1), AI_STRIDE_INIT(4, 4, 4, 40, 40),
  1, &serving_default_dense_32_input0_output_array, NULL)
...
AI_TENSOR_CHAIN_OBJ_DECLARE(
  ...
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &serving_default_dense_32_input0_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &gemm_0_output),
...
AI_NETWORK_OBJ_DECLARE(
  ...
  AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, 2068, 1, 1),
    2068, NULL, NULL),
  AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, 120, 1, 1),
    120, NULL, NULL),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_NLP_IN_NUM, &serving_default_dense_32_input0_output),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_NLP_OUT_NUM, &nl_4_output),
  &gemm_0_layer, 0xbce50aa1, NULL)
...
/******************************************************************************/
AI_DECLARE_STATIC
ai_bool nlp_configure_activations(
  ...
  ai_handle network, ai_network_report* report)
{
  ...
  {
    ai_network_report r = {
      .model_name        = AI_NLP_MODEL_NAME,
      .model_signature   = AI_NLP_MODEL_SIGNATURE,
...
AI_API_ENTRY
ai_error ai_nlp_create_and_init(
  ai_handle* network, const ai_handle activations[], const ai_handle weights[])
{
  ...
  if (ai_nlp_data_params_get(&params) != true) {
    err = ai_nlp_get_error(*network);
    return err;
  }
#if defined(AI_NLP_DATA_ACTIVATIONS_COUNT)
  /* set the addresses of the activations buffers */
  for (ai_u16 idx=0; activations && idx<params.map_activations.size; idx++) {
    AI_BUFFER_ARRAY_ITEM_SET_ADDRESS(&params.map_activations, idx, activations[idx]);
  }
#endif
#if defined(AI_NLP_DATA_WEIGHTS_COUNT)
...
AI_API_ENTRY
ai_handle ai_nlp_destroy(ai_handle network)
{
...
AI_API_ENTRY
ai_i32 ai_nlp_forward(ai_handle network, const ai_buffer* input)
{
